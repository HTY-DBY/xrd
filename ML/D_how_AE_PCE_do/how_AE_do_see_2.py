...
encoding_dim_AE_sets = np.arange(10, np.size(XRD_Data_load, 1), 10)
exist_yes = 0
# encoding_dim_AE_sets = [10, 20]
time_data = pd.read_csv(GobleD().how_AE_time, engine='python')['0'].to_list()
# %%
result_data = []  # 用于存储每次迭代的结果
for idx, encoding_dim_AE_set in enumerate(encoding_dim_AE_sets):
	save_path_AE = os.path.join(GobleD().how_temp, f"{model_name}_how_AEmix_AE_{encoding_dim_AE_set}.joblib")
	save_path_model = os.path.join(GobleD().how_temp, f"{model_name}_how_AEmix_Follow_{encoding_dim_AE_set}.joblib")

	AE_model = joblib.load(save_path_AE)
	model = joblib.load(save_path_model)

	print(f'Running {model_name} {encoding_dim_AE_set}\n--------------------------------')

	XRD_Data_encoded = AE_model.predict_withxrd(XRD_Data_load)
	XRD_Data_recovered = AE_model.recover(XRD_Data_encoded)
	...
	# print(f'ok {encoding_dim_AE_set}')
	print(f'--------------------------------')
result_fin = pd.DataFrame(result_data)
result_fin.to_csv(GobleD().how_AE_result_fin, index=True)
print(f'ok save {GobleD().how_AE_result_fin}')
